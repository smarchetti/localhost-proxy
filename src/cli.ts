...
async function ensureDaemon(): Promise<void> {
  ...
  // The runtime is Node (bin shebang + --target=node build) — Bun's node:http
  // and node:tls gaps (dropped upgrade-socket writes, no SNICallback) rule it
  // out for a proxy. In source mode, run with `node src/lhp.ts` (Node ≥ 23.6
  // executes erasable TypeScript directly).
